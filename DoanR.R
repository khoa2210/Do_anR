...
top3 = x[1:3,]
top3 <- top3[c(1,3)]
other = data.frame('Other',sum(countrytotal$cumconfirmed))
names(other) <- c('Country.Region','confirmed')
top3 = rbind(top3,other)
top3$prop = sprintf((top3$confirmed / sum(top3$confirmed) *100), fmt = '%#.2f')
ggplot(top3, aes(x="",y = confirmed,fill=Country.Region)) +
  geom_bar(stat="identity", width=1, color="white") +
  geom_text(aes(label = prop),
    position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y")+
  scale_fill_brewer(palette="Set3")+
  ggtitle("The pie chart shows the rate of cases of the top 3",
...
#Hình 20
ggplot(top10, aes(x=deaths, y=recovered)) + 
  geom_point(aes(color=Country.Region,size = 5)) + 
  labs(x="deaths", 
       y="recover", 
       title="The graph of the number of deaths and the recover of 10 Country")+ 
  theme_gray()+ # Default theme 
  theme(plot.title = element_text(size=22),axis.text.x= element_text(size=15),
        axis.text.y= element_text(size=15), axis.title=element_text(size=18))
